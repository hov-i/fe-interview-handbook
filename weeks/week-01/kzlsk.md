# 1주차 - kzlsk

## 클로저란 무엇인가요?
- **카테고리**: JavaScript

### 답변

클로저는 함수와 그 함수가 선언될 당시의 렉시컬 환경의 조합입니다.
내부 함수가 외부 함수를 참조하면, 외부 함수의 실행이 끝나도 그 변수가 사라지지 않고 남아있어 계속 참조할 수 있습니다.

### 꼬리질문

- var와 let에서 클로저 동작이 다른 이유는?
- React Hooks에서 클로저가 어떤 역할을 하나요?

---

## 호이스팅이란 무엇인가요?
- **카테고리**: JavaScript

### 답변

호이스팅은 변수와 함수의 선언문이 해당 스코프의 최상단으로 끌어올려지는 현상을 뜻하며 초기화되기 전에 변수와 함수에 접근이 가능합니다.
단, let과 const는 TDZ로 인해 접근 시 에러가 발생합니다.

### 꼬리질문

- var, let, const의 호이스팅 동작 차이는 무엇인가요?
- TDZ는 무엇인가요?

---

## 스코프란 무엇인가요?
- **카테고리**: JavaScript

### 답변

스코프는 유효 범위라는 뜻으로, 식별자가 유효한 범위를 뜻합니다.
자바스크립트 엔진은 스코프를 통해 어떤 변수를 참조해야 할 것인지를 결정하기 때문에 식별자를 검색할 때 사용하는 규칙이라고도 할 수 있습니다.

### 꼬리질문

- 렉시컬 스코프와 동적 스코프의 차이는 무엇인가요?
- 블록 스코프와 함수 스코프의 차이는 무엇인가요?

---

## 동기와 비동기의 차이는 무엇인가요?
- **카테고리**: JavaScript

### 답변

동기 처리 방식은 현재 실행 중인 작업이 종료될 때까지 기다렸다가 다음 작업이 실행되는 방식을 뜻하고, 비동기 처리 방식은 현재 실행 중인 작업이 종료되지 않은 상태여도 다음 작업을 곧바로 실행하는 방식을 뜻합니다.

### 꼬리질문

- 이벤트 루프가 동기/비동기 처리에 어떻게 관여하나요?
- Promise와 async/await의 차이는 무엇인가요?

---

## Virtual DOM이란 무엇인가요?
- **카테고리**: React

### 답변

Virtual DOM은 실제 DOM의 가벼운 복사본을 메모리에 유지하는 개념입니다.
상태가 변경되면 새로운 Virtual DOM 트리를 생성하고, 이전 트리와 비교(diffing)하여
변경된 부분만 실제 DOM에 반영(reconciliation)합니다.

이를 통해 불필요한 DOM 조작을 줄여 성능을 최적화합니다.

### 꼬리질문

- React Fiber는 무엇인가요?
- key prop이 reconciliation에서 왜 중요한가요?

---

## props와 state의 차이는 무엇인가요?
- **카테고리**: React

### 답변

props는 부모 컴포넌트가 설정하는 값으로 자식 컴포넌트는 해당 props를 읽기 전용으로만 사용할 수 있지만, state는 자식 컴포넌트 내부에서 값을 변경할 수 있습니다.

### 꼬리질문

- props drilling 문제를 어떻게 해결할 수 있나요?
- state를 변경했는데 화면이 바로 안 바뀌는 경우가 있는 이유는 무엇인가요?

---

## 리렌더링이 발생하는 조건은 무엇인가요?
- **카테고리**: React

### 답변

리액트 컴포넌트는 state 변경, props 변경, 부모 컴포넌트 리렌더링, Context 값 변경 시 리렌더링이 발생합니다.

### 꼬리질문

- 불필요한 리렌더링을 방지하는 방법은 무엇인가요?
- React.memo와 useMemo, useCallback의 차이는 무엇인가요?

---

## HTTP와 HTTPS의 차이는 무엇인가요?
- **카테고리**: Network

### 답변

HTTPS는 HTTP에 TLS/SSL 암호화 계층을 추가한 프로토콜입니다.

| 구분   | HTTP   | HTTPS   |
| ------ | ------ | ------- |
| 포트   | 80     | 443     |
| 암호화 | 없음   | TLS/SSL |
| 인증서 | 불필요 | 필요    |

HTTPS는 데이터를 암호화하여 중간자 공격을 방지하고, 데이터 무결성을 보장합니다.

### 꼬리질문

- TLS 핸드셰이크 과정을 설명해주세요.
- HSTS란 무엇인가요?

---

## CORS란 무엇인가요?
- **카테고리**: Network

### 답변

CORS란 교차 출처 공유라는 의미로 브라우저가 다른 출처로의 요청을 허용할지 판단하는 보안 메커니즘입니다.
브라우저는 기본적으로 동일 출처 정책에 따라 다른 출처로의 요청을 차단하는데, 서버가 응답 헤더에 허용 출처를 명시하면 브라우저가 이를 확인하고 응답을 허용합니다.

### 꼬리질문

- CORS 에러가 발생했을 때 프론트엔드에서 해결할 수 있는 방법은?
- Preflight 요청이란 무엇인가요?

---

## Flex와 Grid를 사용하는 이유는 무엇인가요?
- **카테고리**: CSS

### 답변

flex와 grid는 레이아웃을 쉽게 잡기 위한 속성입니다.

flex는 1차원 레이아웃에 최적화된 속성으로 컨테이너와 아이템 개념을 사용해 한 방향으로 요소를 정렬하고, 요소의 크기가 불분명하거나 동적인 경우에도 효율적으로 배치할 수 있습니다.

grid는 2차원 레이아웃에 최적화된 속성으로 간격과 비율만 지정하면 행과 열을 동시에 제어할 수 있기 때문에 복잡한 레이아웃을 간단하게 구성할 수 있고, 브라우저 창 크기가 변해도 자동으로 반응하기 때문에 반응형 레이아웃을 만들기 쉽습니다.

### 꼬리질문

- Flex와 Grid를 어떤 상황에서 어떻게 사용하는게 더 적합한가요?
- flex에서 justify-content와 align-items의 차이는 무엇인가요?

---

##  this란?
- **카테고리**: JavaScript

### 답변

this는 함수가 호출되는 방식에 따라 동적으로 결정되는 실행 컨텍스트 참조입니다.
전역에서는 window, 메서드 호출 시에는 해당 객체, 생성자 호출 시에는 새 인스턴스를 가리킵니다.

### 꼬리질문

- bind, call, apply의 차이는?
- 이벤트 리스너 콜백에서 this가 가리키는 것은?

---

## 화살표 함수와 일반 함수의 차이는?
- **카테고리**: JavaScript

### 답변

화살표 함수는 this 선언 시점 상위 스코프에 고정되고, arguments 객체가 없으며, new 생성자로 호출합니다.
일반 함수는 호출 방식에 따라 this가 동적으로 결정됩니다.

### 꼬리질문

- 화살표 함수를 객체 메서드로 쓰면 안 되는 이유는?
- 리액트에서 이벤트 핸들러를 화살표 함수로 쓰는 이유는?

---

## 실행 컨텍스트란?
- **카테고리**: JavaScript

### 답변

코드가 실행되는 환경을 추상화한 객체로, 변수 환경, 렉시컬 환경, this 바인딩으로 구성됩니다.
함수 호출마다 새로 생성되어 콜스택에 쌓입니다.

### 꼬리질문

- 클로저와 실행 컨텍스트는 어떤 관계인가요?
- 호이스팅이 실행 컨텍스트와 어떤 관계인가요?

---

##  Promise란 무엇이고 왜 사용하나요?
- **카테고리**: JavaScript

### 답변

비동기 작업의 성공/실패를 나타내는 객체로, pending, fulfilled, rejected 총 세 가지 상태를 가집니다.
콜백 지옥을 해결하고 .then() 체이닝으로 가독성 좋은 비동기 처리가 가능합니다.

### 꼬리질문

- Promise.all과 Promise.allSettled의 차이는?
- async/await은 Promise와 어떤 관계인가요?

---

## 리액트를 사용하는 이유는?
- **카테고리**: React

### 답변

상태가 변경될 때 DOM을 자동으로 업데이트해주고, 컴포넌트 단위 재사용이 가능합니다.
가상 DOM으로 실제 DOM 조작을 최소화해 성능도 효율적으로 관리할 수 있습니다.

### 꼬리질문

- 가상 DOM이 실제로 성능상 항상 빠른가요?
- 컴포넌트 단위로 개발하면 어떤 장점이 있나요?

---

## 리액트의 컴포넌트 라이프 사이클은?
- **카테고리**: React

### 답변

마운트 > 업데이트 > 언마운트 세 단계로 나뉩니다.
함수형 컴포넌트에서는 useEffect로 제어하며, 의존성 배열로 실행 시점을 조정합니다.

### 꼬리질문

- useEffect clenup 함수는 언제 실행되나요?
- 마운트와 언마운트가 각각 어떤 시점인가요?

---

## 리액트에서 이벤트를 어떻게 핸들링하나요?
- **카테고리**: React

### 답변

JSX에서 onClick, onChange 같은 camelCase 속성으로 핸들러를 연결합니다.
리액트는 내부적으로 이벤트 위임 방식으로 루트에 등록해 관리합니다.

### 꼬리질문

- 리액트의 합성 이벤트란 무엇인가요?
- e.preventDefault(), e.stopPropagination() 차이는?

---

## 타입스크립트를 사용하는 이유는?
- **카테고리**: TypeScript

### 답변

컴파일 타임에 타입 오류를 잡아 런타임 에러를 줄이고, IDE 자동 완성과 타입 추론으로 개발 생산성이 높아집니다.

### 꼬리질문

- any 타입을 쓰면 안 되는 이유는?
- 타입 가드란?

---

## TypeScript에서 type과 interface의 차이는?
- **카테고리**: TypeScript

### 답변

interface는 선언 병합이 가능하고, extends로 확장합니다.
type은 유니온, 인터섹션, 조건부 타입 같은 복잡한 타입 조합이 가능합니다.

### 꼬리질문

- 어떤 상황에서 type을 선택하나요?
- 선언 병합이 실제로 쓰이는 경우는?

---

## 브라우저가 HTML을 받아서 화면에 그리기까지의 과정을 설명해주세요.
- **카테고리**: CS

### 답변

HTML 파싱 > DOM 트리 생성 > CSS 파싱 > CSSOM 생성 > Render Tree 결합 > Layout(reflow) > paint > composite 순서로 진행

### 꼬리질문

- reflow와 repaint의 차이와 최소화하는 방법은?
- <script> 태그 위치가 렌더링에 영향을 주는 이유는?

