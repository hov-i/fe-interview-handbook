...
| 질문 | 답변 요약 | 꼬리질문 |
|------|----------|---------|
| **스코프란 무엇인가요?** | 식별자가 유효한 범위. JS 엔진이 변수를 참조할 때 사용하는 규칙 | ▸ 렉시컬 스코프와 동적 스코프의 차이는?<br>▸ 블록 스코프와 함수 스코프의 차이는? |
| **실행 컨텍스트란?** | 소스 코드를 실행하는 데 필요한 환경을 제공하고 결과를 관리하는 영역 | ▸ 클로저와 실행 컨텍스트는 어떤 관계?<br>▸ 호이스팅이 실행 컨텍스트와 어떤 관계? |
| **클로저란 무엇인가요?** | 함수가 선언된 렉시컬 환경을 기억하여 스코프 밖에서도 접근 가능한 기능 | ▸ 클로저가 메모리 누수를 일으킬 수 있나요?<br>▸ 모듈 패턴은 어떻게 구현?<br>▸ var와 let에서 클로저 동작이 다른 이유는? |
| **동기와 비동기의 차이는?** | 동기는 작업 종료까지 대기, 비동기는 종료 전에도 다음 작업 실행 | ▸ 이벤트 루프가 동기/비동기 처리에 어떻게 관여?<br>▸ Promise와 async/await의 차이는? |
| **호이스팅이란 무엇인가요?** | 변수·함수 선언이 스코프 최상단으로 끌어올려지는 현상 | ▸ var, let, const의 호이스팅 동작 차이는?<br>▸ TDZ는 무엇인가요? |
