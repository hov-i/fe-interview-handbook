...
클로저는 함수와 그 함수가 선언될 당시의 렉시컬 환경의 조합으로, 함수가 선언된 스코프를 기억하여 해당 스코프 밖에서 실행되더라도 그 환경에 접근할 수 있는 기능입니다. 내부 함수가 외부 함수를 참조하면, 외부 함수의 실행이 끝나도 그 변수가 사라지지 않고 계속 참조할 수 있습니다. 

예를 들어, 아래의 코드에서 `outer` 함수는 `count` 변수를 정의하고, 이를 반환하는 `inner` 함수를 통해 클로저를 생성합니다:
...
- **핵심**: 외부 함수의 변수 `count`가 내부 함수 `inner`의 클로저에 의해 유지되어, `counter`를 호출할 때마다 `count`의 값이 증가합니다.
