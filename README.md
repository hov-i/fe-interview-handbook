...
클로저는 함수와 그 함수가 선언될 당시의 렉시컬 환경의 조합으로, 함수가 선언된 스코프를 기억하여 해당 스코프 밖에서 실행되더라도 그 환경에 접근할 수 있는 기능입니다. 내부 함수가 외부 함수를 참조하면, 외부 함수의 실행이 끝난 후에도 그 변수는 사라지지 않고 계속 참조할 수 있습니다.
...
여기서 외부 함수 `outer`의 변수 `count`는 내부 함수 `inner`의 클로저에 의해 유지되며, `counter`를 호출할 때마다 `count`의 값이 증가합니다.
