...
클로저는 함수와 그 함수가 선언될 당시의 렉시컬 환경의 조합으로, 함수가 선언된 스코프의 변수에 접근할 수 있는 기능입니다. 이는 내부 함수가 외부 함수를 참조할 때 발생하며, 외부 함수의 실행이 끝난 후에도 해당 변수는 사라지지 않고 계속 참조할 수 있습니다.

예를 들어, 다음과 같은 코드에서 클로저가 어떻게 작동하는지 볼 수 있습니다:
...
위 코드에서 `inner` 함수는 `outer` 함수의 `count` 변수를 클로저로 유지하고 있어, `outer` 함수가 종료된 후에도 `count`에 접근할 수 있습니다. 

- **핵심**: 클로저는 외부 함수의 변수를 내부 함수가 유지하게 하여, 상태를 기억하고 관리할 수 있는 강력한 도구입니다.
